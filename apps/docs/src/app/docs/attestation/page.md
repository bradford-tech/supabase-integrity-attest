...
5. Your server calls `verifyAttestation()` to validate everything and extract the device's public key.

{% diagram name="attestation-flow" alt="Attestation data flow: iOS App generates a key pair in the Secure Enclave, attests it with Apple Servers, and sends the attestation to your server for verification." /%}
...
After a successful attestation, persist the `publicKeyPem`, `receipt`, and `signCount` for this device. You'll need the public key and counter for every subsequent assertion verification. The receipt is needed if you later want to use Apple's fraud assessment endpoints.
{% /callout %}

...
3. **Hash the challenge** — Compute `clientDataHash = SHA-256(challenge)`. Client SDKs (Expo's `attestKeyAsync`, native `DCAppAttestService` wrappers) hash the challenge before passing to Apple, so the server must hash it too for the nonce to match. If you use the [`withAttestation` wrapper](/docs/with-attestation) instead of calling `verifyAttestation` directly, this step is handled for you automatically.

4. **Call [`verifyAttestation()`](/docs/verify-attestation)** — Pass the `appInfo`, `keyId`, `clientDataHash` (the SHA-256 hash from the previous step, NOT the raw challenge), and the attestation.
...
6. **Handle errors** — `verifyAttestation()` throws `AttestationError` with a typed `code`. See [Types & error codes](/docs/types-and-error-codes) for the full list.

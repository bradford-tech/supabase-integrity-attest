...
--
-- Newer Supabase projects no longer auto-grant DML on new tables to
-- anon/authenticated/service_role. RLS-with-no-policies still blocks anon
-- and authenticated, but the service role now needs explicit grants or
-- every edge-function query fails with "permission denied".
